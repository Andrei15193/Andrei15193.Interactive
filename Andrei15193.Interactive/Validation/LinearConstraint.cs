...
namespace Andrei15193.Interactive.Validation
{
    ...
    /// satisfied, a collection containing <see cref="ValidationError"/>s from all
    ...
    /// <see cref="ValidationError"/> that has been returned thus far is returned by the
    /// linear constraint.
    /// </para>
    /// <para>
    ...
    /// <see cref="ValidationError"/>s found so far are returned.
    /// </para>
    /// <code>
    ...
    /// 
    ///             return new[] { new ValidationError("Cannot be null") };
    ///         })
    ...
    ///                 return Enumerable.Empty&lt;ValidationError&gt;();
    /// 
    ///             return new[] { new ValidationError("Cannot exceed 255 characters") };
    ///         })
    ///     .AndEndedWith(
    ///         value =&gt;
    ///         {
    ...
    ///                 return Enumerable.Empty&lt;ValidationError&gt;();
    /// 
    ///             return new[] { new ValidationError("Must start with \"Test\"") };
    ///         });
    /// 
    ...
    public static class LinearConstraint
    {
        ...
        /// <summary>
        /// Creates a <see cref="ILinearConstraintBuilder{TValue}"/> builder containing the provided
        ...
        /// </param>
        /// <returns>
        /// Returns a <see cref="ILinearConstraintBuilder{TValue}"/> builder that can be used to create a
        /// <see cref="LinearConstraint"/>.
        /// </returns>
        ...
        /// <summary>
        /// Creates a <see cref="ILinearConstraintBuilder{TValue}"/> builder containing an
        ...
        /// <summary>
        /// Creates a <see cref="ILinearConstraintBuilder{TValue}"/> containing an
        /// <see cref="IConstraint{TValue}"/> that has been created from the provided
        /// async callback.
        /// </summary>
        /// <typeparam name="TValue">
            ...
            : ILinearConstraintBuilder<TValue>
        {
                ...
                : Constraint<TValue>
            {
                ...
                {
                    var validationErrors = Enumerable.Empty<ValidationError>();

                    using (var constraintBatch = _constraintBatches.GetEnumerator())
                        while (!validationErrors.Any() && constraintBatch.MoveNext())
                        {
                            validationErrors = await _GetValidationErrorsFrom(
                ...
                private static async Task<IEnumerable<ValidationError>> _GetValidationErrorsFrom(IEnumerable<IConstraint<TValue>> constraints, TValue value, CancellationToken cancellationToken)
                {
                    var validationErrors = new List<ValidationError>(0);

                    foreach (var constraint in constraints)
                        validationErrors.AddRange(await constraint.CheckAsync(value, cancellationToken));
